...
실행 흐름 (설계 문서 기준):
  0. 중복 실행 방지
  1. BigQuery 데이터 추출 (+ phase_name 확인, 추출 기간 확인)
  2. 검수 (날짜 / Global / 행수)
  3. 미리보기 산출물 생성
  4. 로깅
...
from period import confirm_period, resolve_period
...
def main() -> int:
    ...
    # 1-2. 추출 기간 확인 (마지막 성공 실행 기준으로 계산, 시작일은 직접 고칠 수 있음)
    period = confirm_period(resolve_period())
    if period is None:
        print("추출 기간을 다시 확인한 뒤 재실행해주세요.")
        return 0

    # 1. BigQuery 추출
    with Timer() as timer:
        try:
            df = run_extraction(period.start, period.end)
    ...
    # 2. 검수
    validations = run_all_validations(df, period.start, period.end)
    ...
    print("\n여기까지 자동화 완료. 이후 SharePoint append / 파일명 변경 / Slack 알림은 수동으로 진행해주세요.")
    return 0
